...
export function AlbumCover({ design, title, onChangeDesign, onChangeTitle, isThumbnail }: AlbumCoverProps) {
  const [isEditingTitle, setIsEditingTitle] = useState(false);
  const [editValue, setEditValue] = useState(title);

  function handleTitleClick(e: React.MouseEvent) {
    if (isThumbnail || !onChangeTitle) return;
    e.stopPropagation();
    setEditValue(title);
  ...
  return (
    <div
      ...
      onClick={!isEditingTitle ? onChangeDesign : undefined}
      style={{ cursor: (onChangeDesign && !isEditingTitle) ? 'pointer' : undefined }}
    >
      <svg viewBox="0 0 540 720" className={styles.coverSvg}>
        ...
        {/* Title area via foreignObject */}
        <foreignObject x="50" y="265" width="440" height="150">
          {isEditingTitle ? (
            <textarea
              value={editValue}
            ...
            <div
              ...
              style={{
                ...
                fontWeight: 'bold',
                color: '#555',
                cursor: (!isThumbnail && onChangeTitle) ? 'text' : 'default',
                fontFamily: 'sans-serif',
                lineHeight: 1.4,
                userSelect: 'none',
              }}
